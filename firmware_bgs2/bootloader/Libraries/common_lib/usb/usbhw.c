...
void USB_Init (void) {
	...
	//AFIO->MAPR |= AFIO_MAPR_SWJ_CFG_JTAGDISABLE;//only user when PB3,pB4,PB5,PA15 user for another funtion
  /* Control USB connecting via SW */
//   RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;                 /* enable clock for GPIOB */
//   GPIOB->CRL &= ~0x0000F000;                /* clear port PB3 */
//   GPIOB->CRL |=  0x00007000;                /* PB3 General purpose output open-drain, max speed 50 MHz */
// 	GPIOB->BRR  = (1<<3); 
	USB_Connected = 0;
