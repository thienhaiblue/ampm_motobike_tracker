...
BYTE HID_receivedBuf[64];
BYTE HID_receivedLength;
...
BOOL HID_GetReport (void) {
  ...
  switch (SetupPacket.wValue.WB.H) {
    case HID_REPORT_INPUT:
			GetInReport();
  //   numBytesRead = GetInReport(hidBufIn);
//		if(numBytesRead)
	//		USB_WriteEP (0x81,hidBufIn,numBytesRead);
      break;
...
//	uint8_t numBytesRead;
  ...
  switch (SetupPacket.wValue.WB.H) {
	...
	//		numBytesRead = USB_ReadEP(0x01,hidBufOut);
  //    SetOutReport(hidBufOut,numBytesRead);
		SetOutReport();
      break;
    case HID_REPORT_FEATURE:
      return (FALSE);          /* Not Supported */
