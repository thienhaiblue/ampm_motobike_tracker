...
{	
	...
	// 4. Protocol identifiers.
	output_buffer[output_buffer_length++] = 0x00;  // TP-PID: Protocol identifier.
	output_buffer[output_buffer_length++] = 0x08;  // TP-DCS: Data coding scheme.
	output_buffer[output_buffer_length++] = 0xB0;  // TP-VP: Validity: 10 days

	// 5. SMS message.
	
	
	if (sms_len > SMS_MAX_7BIT_TEXT_LENGTH)
		return -1;
	output_buffer[output_buffer_length++] = sms_len;
	memcpy(&output_buffer[output_buffer_length],sms_text,sms_len);
	output_buffer_length += sms_len;
	return output_buffer_length;
}
	       ...
	       char* output_sms_text, int sms_text_size)
{
	...
	char gmt_offset ;
	int sms_start;
	int output_sms_text_length;
	int decoded_sms_text_size;
	...
	sender_number_length = buffer[sms_deliver_start + 1];
	...
	sms_pid_start = sms_deliver_start + 3 + (buffer[sms_deliver_start + 1] + 1) / 2;
	...
	gmt_offset   = SwapDecimalNibble(buffer[sms_pid_start + 8]);
	...
	output_sms_text_length = buffer[sms_start];
	if (sms_text_size < output_sms_text_length) return -1;  // Cannot hold decoded buffer.

	decoded_sms_text_size = DecodePDUMessage(buffer + sms_start + 1, buffer_length - (sms_start + 1),
