...
uint8_t VCOM_TxBuff[64];
RINGBUF VCOM_TxRingBuff;

uint8_t VCOM_RxBuff[64];
RINGBUF VCOM_RxRingBuff;
...
uint32_t CDC_SetLineCoding (void) {
  ...
  CDC_LineCoding.bDataBits   =  EP0Buf[6];
	...
	if(CDC_LineCoding.bCharFormat > 1) CDC_LineCoding.bCharFormat = 1;
	if(CDC_LineCoding.bParityType > 4) CDC_LineCoding.bParityType = 4;
	if(CDC_LineCoding.bDataBits > 8) CDC_LineCoding.bDataBits = 8;
	if(CDC_LineCoding.bDataBits < 5)	CDC_LineCoding.bDataBits = 5;
	
// 	UARTConfigStruct.Baud_rate = CDC_LineCoding.dwDTERate;
// 	UARTConfigStruct.Databits = (UART_DATABIT_Type)(CDC_LineCoding.bDataBits - 5);
// 	UARTConfigStruct.Parity = (UART_PARITY_Type)CDC_LineCoding.bParityType;
// 	UARTConfigStruct.Stopbits =  (UART_STOPBIT_Type)CDC_LineCoding.bCharFormat;
// 	
// 	UART1_Init(&UARTConfigStruct);
  return (TRUE);
}
...
void CDC_BulkIn(void) {
  ...
  // ... add code to check for overwrite
	while(RINGBUF_Get(&VCOM_TxRingBuff,&c) == 0)
	{
			BulkBufIn[numBytesRead] = c;
			numBytesRead++;
			if(numBytesRead >= USB_CDC_BUFSIZE) break;
	}
  // send over USB
  if (numBytesRead > 0) 
	{
...
void CDC_BulkOut(void) {
  ...
  //CDC_WrOutBuf ((char *)&BulkBufOut[0], &numBytesRead);
	...
	{
		RINGBUF_Put(&VCOM_RxRingBuff,BulkBufOut[i]);
	}
}
...
unsigned short CDC_GetSerialState (void) {
  ...
  CDC_SerialState = 0;
	//temp = ((LPC_UART1->MSR<<8)|LPC_UART1->LSR) & 0xE01E;    // update linestate
 ...
 *---------------------------------------------------------------------------*/
void VCOM_Serial2Usb(uint8_t* c) 
{
    if (CDC_DepInEmpty) 
		{
      CDC_DepInEmpty = 0;
 ...
 *---------------------------------------------------------------------------*/
uint8_t VCOM_Usb2Serial(uint8_t* buff) 
{
  int  numBytesToRead,numBytesRead, numAvailByte;
  CDC_OutBufAvailChar (&numAvailByte);
  if (numAvailByte > 0) {
      numBytesToRead = numAvailByte > 32 ? 32 : numAvailByte;
      numBytesRead = CDC_RdOutBuf ((char *)&buff[0], &numBytesToRead);
			return numBytesRead;
  }
	return 0;
}
