...
{
	uint32_t di,si;
	uint16_t w,d,*wo = (uint16_t *)out;
	const uint16_t *p = (const uint16_t *)path;
	uint8_t *p1,*p2;
	 si = di = 0;
	for (;;) {
		...
		w = uni_convert(w, 1,1);			/* Convert ANSI/OEM to Unicode */
		if (!w) return di;	/* Reject invalid code */
