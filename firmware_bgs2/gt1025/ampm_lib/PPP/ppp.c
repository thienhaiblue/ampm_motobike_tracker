...
uint32_t PPP_ManageLink(void)
{
	switch(pppPhase){
		case PPP_PHASE_DEAD:	
			if(COMM_ModemIsOnline() && COMM_pppIsOpened())
			{
				COMM_pppClrOpened();
				COMM_ModemSetOffline();
			}
			pppPhase = PPP_PHASE_WAIT_ONLINE;
		break;
		case PPP_PHASE_WAIT_ONLINE:
...
void PPP_Send(U8* packet, U16 len)
{
	...
	RINGBUF_Put(commTxRingBuf,0x7E);
	while(len--) {
		if(*packet < 0x20){
			if(pppUseESC){
				RINGBUF_Put(commTxRingBuf,0x7D);
				RINGBUF_Put(commTxRingBuf,*packet ^ 0x20);
			}else{
				RINGBUF_Put(commTxRingBuf,*packet);
			}
		}else{
			switch(*packet){
				case 0x7E:
					RINGBUF_Put(commTxRingBuf,0x7D);
					RINGBUF_Put(commTxRingBuf,0x5E);
					break;
	
				case 0x7D:
					RINGBUF_Put(commTxRingBuf,0x7D);
					RINGBUF_Put(commTxRingBuf,0x5D);
					break;
